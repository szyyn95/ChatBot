""" 
Original code created by Chip Huyen as the starter code for assignment 3, class CS 20SI: 
"TensorFlow for Deep Learning Research"
https://github.com/chiphuyen/stanford-tensorflow-tutorials/tree/master/assignments/chatbot

This code contains the code to do the pre-processing for the Cornell Movie-Dialogs Corpus.
This code contains major modification since the original code doesn't work on Python 3.6 due to some byte type issue
This code contains other modifications to further clean the data
"""
...
def prepare_dataset(questions, answers):
    ...
    for i in range(len(questions)):
        if i in test_ids:
            files[2].write(bytearray(questions[i] + '\n', "cp437"))
            files[3].write(bytearray(answers[i] + '\n', "cp437"))
        else:
            files[0].write(bytearray(questions[i] + '\n', "cp437"))
            files[1].write(bytearray(answers[i] + '\n', "cp437"))
...
def basic_tokenizer(line, normalize_digits=True):
    ...
    Variable line here is in byte type
    """
    # ADD MORE HERE
        ...
        for word in sorted_vocab:
            if vocab[word] < config.THRESHOLD:
                with open('config.py', 'ab') as cf:
                    if filename[-3:] == 'enc':
                        cf.write(bytearray('ENC_VOCAB = ' + str(index) + '\n', "cp437"))
                    else:
                        cf.write(bytearray('DEC_VOCAB = ' + str(index) + '\n', "cp437"))
                break
            f.write(word + b'\n')
            index += 1
...
def token2id(data, mode):
    ...
    for line in lines:
        ...
        out_file.write(bytearray(' '.join(str(id_) for id_ in ids) + '\n', "cp437"))
